...
@IBDesignable
open class RCISOTPDotView: UIView {
    ...
    open override func draw(_ rect: CGRect) {
        ...
        for (index, position) in positions.enumerated() {
            if index < inputDotCount {
                let pathToFill = UIBezierPath(rectWithCenter: position, radius: (radius + borderWidth / 2), lineWidth: borderWidth)
                pathToFill.fill()
            } else {
                let pathToStroke = UIBezierPath(rectWithCenter: position, radius: radius, lineWidth: borderWidth)
                pathToStroke.stroke()
            }
        }
    }
...
private extension RCISOTPDotView {
    //MARK: Animation
...
internal extension UIBezierPath {
    convenience init(rectWithCenter center: CGPoint, radius: CGFloat, lineWidth: CGFloat) {
        self.init(roundedRect: CGRect(x: center.x, y: center.y, width: 30, height: 30), cornerRadius: 5)
