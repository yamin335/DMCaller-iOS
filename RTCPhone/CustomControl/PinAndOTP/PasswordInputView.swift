...
//    }
    ...
    fileprivate func updateUI() {
        ...
        //update label
        label.text = numberString
        label.font = UIFont.systemFont(ofSize: radius * fontSizeRatio, weight: UIFont.Weight.light)
        label.textColor = textColor
...
//        circleView.layer.borderWidth = isVibrancyEffect ? borderWidth : 0
        ...
        circleView.backgroundColor = circleBackgroundColor
        //circle view border
        circleView.layer.borderWidth = isVibrancyEffect ? borderWidth : 1
        
        
        let path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: width, height: height), byRoundingCorners: .allCorners, cornerRadii: CGSize(width: 10.0, height: 10.0))
        ...
        //update mask
//        let path = UIBezierPath(arcCenter: center, radius: 10, startAngle: 0, endAngle: 2.0 * CGFloat(Double.pi), clockwise: false)
//        let maskLayer = CAShapeLayer()
//        maskLayer.path = path.cgPath
//        layer.mask = maskLayer
