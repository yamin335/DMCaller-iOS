...
/**
 An HoshiTextField is a subclass of the TextFieldEffects object, is a control that displays an UITextField with a customizable visual effect around the lower edge of the control.
 */
@IBDesignable open class RCTPlaceHolderTextField: TextFieldEffects {
    ...
    private let borderThickness: (active: CGFloat, inactive: CGFloat) = (active: 2, inactive: 2)
    private let placeholderInsets = CGPoint(x: 0, y: 0) //15,0
    private let textFieldInsets = CGPoint(x: 0, y: 12) //15,12
    private let inactiveBorderLayer = CALayer()
    private let activeBorderLayer = CALayer()
    private var activePlaceholderPoint: CGPoint = CGPoint.zero
    ...
    private func layoutPlaceholderInTextRect() {
        ...
        placeholderLabel.frame = CGRect(x: textRect.origin.x, y: textRect.height/2,
                                        width: placeholderLabel.bounds.width, height: placeholderLabel.bounds.height)
        activePlaceholderPoint = CGPoint(x: placeholderLabel.frame.origin.x, y: placeholderLabel.frame.origin.y - placeholderLabel.frame.size.height - placeholderInsets.y)
