...
public class PasswordRule : RegexRule {
    ...
    // static let regex = "^(?=.*?[A-Z])(?=.*?[0-9])(?=.*?[a-z]).{10,}$"
    
    // 10 characters. One uppercase. One Lowercase. One number one special character.
    static let regex = "^(?=.*[a-z])(?=.*[A-Z])(?=.*?[0-9])(?=.*[!#$%&()?'\"*+,-./:;<=>@_|~])[A-Za-z[0-9]#$%&()!*+?'\",-./:;<=>@_|~]{10,30}"
